// server/server-context.js
// Context module: chunk upload, merge, whisper, gpt, tts.
// Exports { router, prefix, init }.
// Uses process.cwd() for storing chunks.

import express from "express";
import fs from "fs";
import path from "path";
import FormData from "form-data";
import { WebSocketServer } from "ws";

export const prefix = "/context";
export const router = express.Router();

const OPENAI_API_KEY = process.env.OPENAI_API_KEY || "";
const CHUNKS_DIR = process.cwd(); // <-- use process.cwd() as requested

const rawMiddleware = express.raw({ type: "application/octet-stream", limit: "30mb" });

// helper: convert Float32 -> WAV buffer
function floatToWav(f32, sampleRate) {
  const buffer = Buffer.alloc(44 + f32.length * 2);
  const dv = new DataView(buffer.buffer);
  dv.setUint32(0, 0x52494646, false);
  dv.setUint32(4, 36 + f32.length * 2, true);
  dv.setUint32(8, 0x57415645, false);
  dv.setUint32(12, 0x666d7420, false);
  dv.setUint32(16, 16, true);
  dv.setUint16(20, 1, true);
  dv.setUint16(22, 1, true);
  dv.setUint32(24, sampleRate, true);
  dv.setUint32(28, sampleRate * 2, true);
  dv.setUint16(32, 2, true);
  dv.setUint16(34, 16, true);
  dv.setUint32(36, 0x64617461, false);
  dv.setUint32(40, f32.length * 2, true);
  let off = 44;
  for (let i = 0; i < f32.length; i++) {
    let s = Math.max(-1, Math.min(1, f32[i]));
    dv.setInt16(off, s < 0 ? s * 0x8000 : s * 0x7fff, true);
    off += 2;
  }
  return Buffer.from(buffer);
}

function listChunks(session) {
  if (!fs.existsSync(CHUNKS_DIR)) return [];
  return fs.readdirSync(CHUNKS_DIR)
    .filter(f => f.startsWith(`${session}_chunk_`))
    .sort((a,b)=> {
      const na = +(a.match(/chunk_(\d+)/)||[])[1]||0;
      const nb = +(b.match(/chunk_(\d+)/)||[])[1]||0;
      return na - nb;
    });
}

// POST /context/chunk?session=&sampleRate=
router.post("/chunk", rawMiddleware, (req, res) => {
  try {
    const session = String(req.query.session || `sess-${Date.now()}`);
    const sampleRate = parseInt(req.query.sampleRate || "44100", 10);
    if (!req.body || !req.body.byteLength) return res.status(400).json({ error: "no_body" });

    const buf = Buffer.from(req.body);
    const floatArray = new Float32Array(buf.buffer, buf.byteOffset, Math.floor(buf.byteLength / 4));
    const idx = listChunks(session).length;
    const filename = `${session}_chunk_${idx}.wav`;
    const wav = floatToWav(floatArray, sampleRate);
    fs.writeFileSync(path.join(CHUNKS_DIR, filename), wav);
    return res.json({ ok: true, file: filename });
  } catch (e) {
    console.error("[context/chunk]", e && e.message ? e.message : e);
    return res.status(500).json({ error: e && e.message ? e.message : "chunk_error" });
  }
});

// GET /context/merge?session=
router.get("/merge", (req, res) => {
  try {
    const session = String(req.query.session || "");
    if (!session) return res.status(400).send("no session");
    const files = listChunks(session);
    if (!files.length) return res.status(404).send("no chunks");

    const header = 44;
    const first = fs.readFileSync(path.join(CHUNKS_DIR, files[0]));
    const sr = first.readUInt32LE(24);
    const pcms = files.map(f => fs.readFileSync(path.join(CHUNKS_DIR, f)).subarray(header));
    const mergedPCM = Buffer.concat(pcms);

    const outHeader = Buffer.alloc(44);
    outHeader.write("RIFF",0); outHeader.writeUInt32LE(36 + mergedPCM.length,4); outHeader.write("WAVE",8);
    outHeader.write("fmt ",12); outHeader.writeUInt32LE(16,16); outHeader.writeUInt16LE(1,20); outHeader.writeUInt16LE(1,22);
    outHeader.writeUInt32LE(sr,24); outHeader.writeUInt32LE(sr*2,28); outHeader.writeUInt16LE(2,32); outHeader.writeUInt16LE(16,34);
    outHeader.write("data",36); outHeader.writeUInt32LE(mergedPCM.length,40);

    const merged = Buffer.concat([outHeader, mergedPCM]);
    const outFile = `${session}_merged.wav`;
    fs.writeFileSync(path.join(CHUNKS_DIR, outFile), merged);
    return res.json({ ok: true, file: `/${outFile}` });
  } catch (e) {
    console.error("[context/merge]", e && e.message ? e.message : e);
    return res.status(500).send("merge error");
  }
});

// GET /context/whisper?session=&langPair=
router.get("/whisper", async (req, res) => {
  try {
    const session = String(req.query.session || "");
    const langPair = String(req.query.langPair || "");
    if (!session) return res.status(400).send("no session");
    const file = path.join(CHUNKS_DIR, `${session}_merged.wav`);
    if (!fs.existsSync(file)) return res.status(404).send("no file");
    if (!OPENAI_API_KEY) return res.status(500).json({ error: "OPENAI_API_KEY not set" });

    // Use global fetch (Node18+). If not available, ensure node-fetch is installed.
    const form = new FormData();
    form.append("file", fs.createReadStream(file));
    form.append("model", "whisper-1");
    form.append("response_format", "verbose_json");
    form.append("task", "transcribe");

    const r = await (global.fetch ? global.fetch : (await import("node-fetch")).default)("https://api.openai.com/v1/audio/transcriptions", {
      method: "POST",
      headers: { Authorization: `Bearer ${OPENAI_API_KEY}` },
      body: form
    });
    const data = await r.json();
    let detectedLang = data.language || null;
    const text = data.text || "";

    // optional GPT-based language correction if langPair provided
    if (langPair && (!detectedLang || !langPair.split("-").includes(detectedLang))) {
      const [a,b] = langPair.split("-");
      const prompt = `Text: """${text}"""\nDecide which of these two languages it is written in: ${a} or ${b}. Return only one code (${a} or ${b}).`;
      const chk = await (global.fetch ? global.fetch : (await import("node-fetch")).default)("https://api.openai.com/v1/chat/completions", {
        method: "POST",
        headers: { Authorization: `Bearer ${OPENAI_API_KEY}`, "Content-Type": "application/json" },
        body: JSON.stringify({ model: "gpt-4o-mini", messages: [{ role: "user", content: prompt }] })
      });
      const chRes = await chk.json();
      const corrected = chRes.choices?.[0]?.message?.content?.trim().toLowerCase();
      if (corrected && [a,b].includes(corrected)) detectedLang = corrected;
    }

    return res.json({ text, detectedLang });
  } catch (e) {
    console.error("[context/whisper]", e && e.message ? e.message : e);
    return res.status(500).json({ error: e && e.message ? e.message : "whisper_error" });
  }
});

// POST /context/gpt  body: { text, mode, langPair, detectedLang }
router.post("/gpt", express.json(), async (req, res) => {
  try {
    const { text, mode, langPair, detectedLang } = req.body || {};
    if (!text) return res.status(400).send("no text");
    if (!OPENAI_API_KEY) return res.status(500).json({ error: "OPENAI_API_KEY not set" });

    let prompt = text;
    if (mode === "translate" && langPair) {
      const [a,b] = (langPair||"en-ru").split("-");
      const from = detectedLang && [a,b].includes(detectedLang) ? detectedLang : a;
      const to = from === a ? b : a;
      prompt = `Translate from ${from.toUpperCase()} to ${to.toUpperCase()}: ${text}`;
    } else if (mode === "assistant") {
      prompt = `Act as a helpful assistant. Reply naturally: ${text}`;
    }

    const r = await (global.fetch ? global.fetch : (await import("node-fetch")).default)("https://api.openai.com/v1/chat/completions", {
      method: "POST",
      headers: { Authorization: `Bearer ${OPENAI_API_KEY}`, "Content-Type": "application/json" },
      body: JSON.stringify({ model: "gpt-4o-mini", messages: [{ role: "user", content: prompt }] })
    });
    const data = await r.json();
    return res.json({ text: data.choices?.[0]?.message?.content ?? "" });
  } catch (e) {
    console.error("[context/gpt]", e && e.message ? e.message : e);
    return res.status(500).json({ error: e && e.message ? e.message : "gpt_error" });
  }
});

// GET /context/tts?session=&voice=&text=
router.get("/tts", async (req, res) => {
  try {
    const text = String(req.query.text || "");
    const session = String(req.query.session || "sess");
    const voice = String(req.query.voice || "alloy");
    if (!text) return res.status(400).send("no text");
    if (!OPENAI_API_KEY) return res.status(500).json({ error: "OPENAI_API_KEY not set" });

    const r = await (global.fetch ? global.fetch : (await import("node-fetch")).default)("https://api.openai.com/v1/audio/speech", {
      method: "POST",
      headers: { Authorization: `Bearer ${OPENAI_API_KEY}`, "Content-Type": "application/json" },
      body: JSON.stringify({ model: "gpt-4o-mini-tts", voice, input: text })
    });
    const audio = await r.arrayBuffer();
    const file = `${session}_tts.mp3`;
    fs.writeFileSync(path.join(CHUNKS_DIR, file), Buffer.from(audio));
    return res.json({ url: `/${file}` });
  } catch (e) {
    console.error("[context/tts]", e && e.message ? e.message : e);
    return res.status(500).json({ error: e && e.message ? e.message : "tts_error" });
  }
});

// optional: attach WebSocketServer if main server passes server to init()
export function init(app, server) {
  if (!server) {
    console.log("[context] init(): server not passed — WS disabled");
    return;
  }
  const wss = new WebSocketServer({ server, path: `${prefix}/ws` });
  let counter = 1;
  wss.on("connection", (ws) => {
    ws.sessionId = `sess-${counter++}`;
    ws.sampleRate = 44100;
    ws.chunkCounter = 0;
    ws.send(`SESSION:${ws.sessionId}`);
    console.log(`[context/ws] connected ${ws.sessionId}`);

    ws.on("message", (data) => {
      if (typeof data === "string") {
        try {
          const meta = JSON.parse(data);
          if (meta.type === "meta") {
            ws.sampleRate = meta.sampleRate || ws.sampleRate;
            return ws.send(`META_OK:${ws.sampleRate}`);
          }
        } catch (e) {}
        return;
      }
      try {
        const buf = Buffer.from(data);
        const f32 = new Float32Array(buf.buffer, buf.byteOffset, Math.floor(buf.byteLength / 4));
        const wav = floatToWav(f32, ws.sampleRate);
        const filename = `${ws.sessionId}_chunk_${ws.chunkCounter++}.wav`;
        fs.writeFileSync(path.join(CHUNKS_DIR, filename), wav);
        ws.send(`SAVED:${filename}`);
      } catch (e) {
        console.error("[context/ws] save error", e && e.message ? e.message : e);
      }
    });

    ws.on("close", () => console.log(`[context/ws] closed ${ws.sessionId}`));
  });

  console.log(`[context] WebSocket attached at ${prefix}/ws`);
}
